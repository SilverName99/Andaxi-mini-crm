...
export function ClientDetail() {
  ...
  return (
    <div className="animate-fade-up">
      <Link to="/clienti" className="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-slate-500 transition hover:text-violet-600">
        <ArrowLeft className="h-4 w-4" /> Înapoi la clienți
      </Link>

      <Card className="mb-4 overflow-hidden p-0">
        <div className="h-24 bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-500" />
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          { label: 'Recurent lunar', value: formatEur(mrr), sub: settings ? formatRon(mrr, settings.eurRon) : '', gradient: 'from-violet-600 to-fuchsia-500' },
          { label: 'Abonamente active', value: String(subscriptions.filter((s) => s.status === 'ACTIVE').length), sub: `${subscriptions.length} în total`, gradient: 'from-emerald-500 to-teal-500' },
          { label: 'Ore lucrate', value: formatMinutes(minutes), sub: `${workLogs.length} intervenții`, gradient: 'from-rose-500 to-pink-500' },
          { label: 'De facturat', value: formatEur(unbilled), sub: 'abonamente + ore', gradient: 'from-amber-500 to-orange-500' },
        ].map((stat) => (
        ...
        <div className="flex flex-col gap-3">
          {subscriptions.length === 0 ? (
            <Card className="py-10 text-center text-sm text-slate-400">
              Niciun abonament.{' '}
              <button onClick={() => setAddingSub(true)} className="font-semibold text-violet-600 hover:underline">
                Adaugă unul
              </button>
            </Card>
          ) : (
        ...
        <div className="flex flex-col gap-2">
          {workLogs.length === 0 ? (
            <Card className="py-10 text-center text-sm text-slate-400">
              ...
              <button onClick={() => setAddingLog(true)} className="font-semibold text-violet-600 hover:underline">
                Adaugă una
              </button>
            </Card>
          ) : (
