...
import { Avatar, Button, Card, CardTitle, EmptyState, ErrorBlock, Field, LoadingBlock } from '../components/ui';
      ...
      {isLoading ? (
        ...
        <>
          <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
            {[
              { label: 'Total perioadă', value: data.totals.total, gradient: 'from-orange-500 to-amber-500' },
              { label: 'Din abonamente', value: data.totals.recurent, gradient: 'from-amber-400 to-orange-500' },
              { label: 'Din ore', value: data.totals.ore, gradient: 'from-orange-600 to-amber-600' },
              { label: 'De facturat', value: data.totals.deFacturat, gradient: 'from-amber-500 to-orange-600' },
            ].map((stat) => (
            ...
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.months.map((m) => ({ ...m, label: formatMonth(m.month) }))} margin={{ top: 5, right: 8, left: -18, bottom: 0 }}>
                  ...
                  <Bar dataKey="recurent" stackId="a" fill="#f97316" radius={[0, 0, 0, 0]} />
                  <Bar dataKey="ore" stackId="a" fill="#78716c" radius={[10, 10, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Card className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-stone-400">Ore în program normal</p>
                <p className="text-lg font-extrabold text-stone-900">{formatMinutes(data.totals.standardMinutes)}</p>
              </div>
            </Card>
            <Card className="flex items-center gap-4">
              <span className="grid h-12 w-12 place-items-center rounded-2xl bg-stone-200 text-stone-600">
                <Moon className="h-6 w-6" />
              </span>
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-stone-400">Ore în afara programului</p>
                <p className="text-lg font-extrabold text-stone-900">{formatMinutes(data.totals.offHoursMinutes)}</p>
              </div>
            </Card>
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[760px] text-sm">
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {data.rows.map((row) => (
                      <tr key={row.id} className="transition hover:bg-stone-50/70">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <Avatar name={row.company || row.name} color={row.color as AccentColor} size="sm" />
                            <span className="font-semibold text-stone-800">{row.company || row.name}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-right text-stone-600">{formatEur(row.recurent)}</td>
                        <td className="px-4 py-3 text-right text-stone-600">{formatEur(row.ore)}</td>
                        <td className="px-4 py-3 text-right text-stone-500">{formatMinutes(row.minutes)}</td>
                        <td className="px-4 py-3 text-right font-bold text-stone-900">{formatEur(row.total)}</td>
                  ...
                  <tfoot>
                    <tr className="bg-stone-50 text-sm font-bold text-stone-900">
