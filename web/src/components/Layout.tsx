        ...
        <NavLink
          ...
          className={({ isActive }) =>
            cn(
              ...
              isActive ? 'bg-stone-900 text-white shadow-soft' : 'text-stone-600 hover:bg-white hover:text-stone-900',
            )
          }
        >
          {({ isActive }) => (
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10">
        <div className="absolute -left-24 -top-24 h-96 w-96 rounded-full bg-orange-300/40 blur-3xl" />
        <div className="absolute right-0 top-1/4 h-80 w-80 animate-float rounded-full bg-amber-200/40 blur-3xl" />
        <div className="absolute bottom-0 left-1/3 h-96 w-96 rounded-full bg-amber-300/30 blur-3xl" />
      </div>
        ...
        <aside className="sticky top-6 hidden h-[calc(100vh-3rem)] w-64 shrink-0 flex-col justify-between rounded-4xl border border-white/60 bg-white/70 p-4 shadow-soft backdrop-blur-xl lg:flex">
          <div>
            <div className="mb-6 flex items-center gap-3 px-2 pt-2">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 text-lg font-extrabold text-white shadow-glow">
                A
              </span>
              <div>
                <p className="text-base font-extrabold leading-tight text-stone-900">Andaxi</p>
                <p className="text-xs font-medium text-stone-500">mini-CRM</p>
              </div>
            </div>
            <NavItems />
          </div>
          ...
          <div className="rounded-3xl bg-stone-900 p-4 text-white">
            <div className="flex items-center gap-3">
              <span className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 text-sm font-bold">
        ...
        <main className="min-w-0 flex-1 pb-10">
          ...
          <div className="mb-4 flex items-center justify-between rounded-3xl border border-white/60 bg-white/70 px-4 py-3 shadow-card backdrop-blur-xl lg:hidden">
            <div className="flex items-center gap-2.5">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-orange-500 to-amber-500 font-extrabold text-white">
                A
              </span>
              <span className="text-sm font-bold text-stone-900">{current?.label ?? 'Andaxi CRM'}</span>
            </div>
            <button
          ...
          <div className="animate-fade-up absolute inset-y-0 left-0 flex w-72 flex-col justify-between bg-white p-4 shadow-soft">
            <div>
              <div className="mb-6 flex items-center justify-between">
                <span className="text-base font-extrabold text-stone-900">Andaxi mini-CRM</span>
                <button onClick={() => setMobileOpen(false)} className="rounded-xl p-2 hover:bg-stone-100">
                  <X className="h-5 w-5" />
                </button>
              </div>
            ...
            <button
              onClick={() => void logout()}
              className="flex items-center justify-center gap-2 rounded-2xl bg-stone-900 px-3 py-2.5 text-sm font-semibold text-white"
            >
              <LogOut className="h-4 w-4" /> Deconectare
            </button>
          </div>
...
export function PageHeader({
  ...
  return (
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-stone-900 sm:text-3xl">{title}</h1>
