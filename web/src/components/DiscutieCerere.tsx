...
export function DiscutieCerere({ taskId, onClose }: { taskId: string; onClose: () => void }) {
  ...
  const numeClient = data?.client ? data.client.company || data.client.name : 'Clientul';

  /*
   * Textul cu care s-a deschis discuția e tratat ca primul mesaj, de la cine a
   * deschis-o — altfel arăta ca un anunț lipit deasupra conversației.
   */
      ...
      {isLoading || !data ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <ul className="flex max-h-[26rem] flex-col gap-2 overflow-y-auto pr-1">
            {/* textul cererii e primul mesaj al discuției, nu o casetă aparte */}
            {toateMesajele.map((m) => (
              <li
                key={m.id}
                className={cn(
              ...
              >
                ...
                <p className={cn('mt-1 text-[11px]', m.author === 'ADMIN' ? 'text-indigo-200' : 'text-slate-400')}>
                  {m.author === 'ADMIN' ? 'Tu' : m.authorName || numeClient} · {formatDateTime(m.createdAt)}
                </p>
              </li>
            ))}
          </ul>
